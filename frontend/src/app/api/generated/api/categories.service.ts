...
import { CategoriesApiInterface } from './categories.serviceInterface';

@Injectable({
  providedIn: 'root',
})
export class CategoriesApi extends BaseService implements CategoriesApiInterface {
  constructor(
  ...
  public createCategory(
    ...
    if (categoryCreate === null || categoryCreate === undefined) {
      throw new Error(
        'Required parameter categoryCreate was null or undefined when calling createCategory.',
      );
    }
  ...
  public updateCategory(
    ...
    let localVarPath = `/api/v1/categories/${this.configuration.encodeParam({ name: 'categoryId', value: categoryId, in: 'path', style: 'simple', explode: false, dataType: 'number', dataFormat: undefined })}`;
    const { basePath, withCredentials } = this.configuration;
    return this.httpClient.request<CategoryResponse>('patch', `${basePath}${localVarPath}`, {
      context: localVarHttpContext,
      body: categoryUpdate,
      responseType: <any>responseType_,
      ...(withCredentials ? { withCredentials } : {}),
